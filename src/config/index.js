...
class RequestService {
  constructor() {
    /* */
    this.version = '';
    this.urlServer = 'http://localhost:8800/api';
  }

  async post(url, data) {
    try {
      const resp = await axios.post(`${this.urlServer}${url}`, data, {
  ...
  async get(url) {
    try {
      const resp = await axios.get(`${this.urlServer}${url}`, {
  ...
  async put(url, data) {
    try {
      const resp = await axios.put(`${this.urlServer}${url}`, data, {
        withCredentials: true,
      });
      return this.switchCase(resp);
    } catch (error) {
  ...
  async delete(url) {
    try {
      const resp = await axios.delete(`${this.urlServer}${url}`, {
        withCredentials: true,
      });
      return this.switchCase(resp);
    } catch (error) {
